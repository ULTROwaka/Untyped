...
public class UntypedCollection
{
    ...
    private readonly Dictionary<Type, object> _observers = new();
    ...
    public void Add<T>(T item)
    {
        ...
        foreach (var observer in (_observers[type] as List<IObserver<T>>)!)
        {
    ...
    public Subscriber<T> GetSubscriber<T>()
    {
        Type type = typeof(T);

        List<IObserver<T>> list = new();
        _observers.Add(type, list);

        return new Subscriber<T>(list);
    }
}